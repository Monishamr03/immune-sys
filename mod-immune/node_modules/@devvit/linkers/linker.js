...
var _DefaultLinker_instances, _DefaultLinker_resolver, _DefaultLinker_link, _DefaultLinker_linkQuery;
import { LinkedBundle } from '@devvit/protos/types/devvit/runtime/bundle.js';
import { LOCAL_HOSTNAME, NODE_HOSTNAME, PLUGIN_NAME, PLUGIN_OWNER, REDDIT_API_CNAME, } from '@devvit/shared-types/HostnameUtil.js';
...
_DefaultLinker_resolver = new WeakMap(), _DefaultLinker_instances = new WeakSet(), _DefaultLinker_link = async function _DefaultLinker_link(bundle) {
    ...
    const dependencies = await Promise.all(bundle.dependencies.uses.map((query) => __classPrivateFieldGet(this, _DefaultLinker_instances, "m", _DefaultLinker_linkQuery).call(this, query)));
    return {
        bundle: {
...
}, _DefaultLinker_linkQuery = async function _DefaultLinker_linkQuery(query) {
    const use = await __classPrivateFieldGet(this, _DefaultLinker_resolver, "f").resolve(query);
    if (use.error != null)
        return { errors: [use.error] };
    if (use.bundle != null)
    ...
    return { bundle: fakeBundleForPlugin(NonNull(use.plugin)), errors: [] };
};
