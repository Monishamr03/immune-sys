...
import { InspectorDescription, } from '@devvit/protos/types/devvit/plugin/inspector/inspector.js';
import { Empty } from '@devvit/protos/types/google/protobuf/empty.js';
import { PLUGIN_NAME } from '@devvit/shared-types/HostnameUtil.js';
...
export class InMemoryResolver {
    constructor(inspectorDescriptions, defunctPackages = []) {
        ...
        __classPrivateFieldGet(this, _InMemoryResolver_instances, "m", _InMemoryResolver_registerPluginsSync).call(this, {
            descriptions: inspectorDescriptions.descriptions.concat(defunctPackages.map((defunctPackage) => {
                return {
                    provides: [{ fullName: defunctPackage, methods: [], name: '', version: '' }],
                    hostname: 'defunct',
                };
            })),
        });
    ...
    async resolve(query) {
        if (query.versions) {
            console.warn("narrowing the query by version query e.g. '>=2' is not yet supported");
        }
        if (query.owner) {
            console.warn('narrowing the query by owner is not yet supported');
        }
