...
import { devvitScriptFileName, devvitScriptUrl, screenshotScriptFileName, } from '@devvit/shared-types/web-view-scripts-constants.js';
const fallbackScriptUrl = new URL(screenshotScriptFileName, devvitScriptUrl).toString();
export function getScreenshotModuleUrl(scriptSrc) {
...
async function onMessage(event, scriptSrc) {
    const envelope = event.data;
    if (envelope?.type !== 'devvit-message' || envelope.data == null)
        return;
    ...
    emitEffect({ type: EffectType.EFFECT_SCREENSHOT_RESPONSE, screenshot: { dataUrl, error: errorMessage } }, payload.id);
}
export function initScreenshotRequestListener(scriptSrc) {
